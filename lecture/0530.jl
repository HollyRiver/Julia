...
md"""
!!! info "예전에 했었는데.."
	임의의 matrix ${\bf X}={\bf U}{\bf D}{\bf V}^\top$ 로 표현가능할때, ${\bf V}$는 ${\bf X}^\top {\bf X}$의 고유벡터행렬이 된다. 
"""
...
# ╔═╡ 3390bcfe-102b-4ba6-8547-e96faabf1507
md"""
> 어떠한 매트릭스가 변환을 의미할 때, $\bf U, V^{\top}$은 직교좌표계의 이동을, $\bf D$는 스케일링을 나타낸다. 즉, 모든 선형변환을 돌리고, 늘이고, 돌리고로 해석할 수 있다. (기하학적 해석?)
"""
...
md"""
!!! warning "변환을 의미하는 행렬의 분해"
	임의의 매트릭스 ${\bf A}$는 아래와 같이 분해가능하다. 
	
	$${\bf A} = {\bf U}{\bf D}{\bf V}^\top$$

	여기에서 ${\bf D}$를 대각행렬로 가정하지 않는다면, ${\bf U} ,{\bf V}$ 는 언제나 직교행렬로 "선택"할 수 있다. 따라서 임의의 선형변환은 직교변환, 스케일링, 직교번환의 조합으로 재표현 가능하다. 이를 통계학의 관점에서 해석해보자. ${\bf X}$가 데이터를 의미하는 행렬이고 ${\bf A}$는 변환을 의미하는 행렬일때 ${\bf X}{\bf A}$는 
	1. 각 관측치별로(for each ${\boldsymbol x}_i^\top$) 직교변환 (공분산 없애고)
	2. 각 관측치별로(for each ${\boldsymbol x}_i^\top$) 스케일링 (분산 안정화하고)
	3. 각 관측치별로(for each ${\boldsymbol x}_i^\top$) 직교변환 (다시 돌려놓고)
	을 한다는 의미이다.
"""
...
# ╔═╡ 6676b26f-fc71-4f37-91e3-1eaf583355a2
md"""
-- 회전변환의 재해석

1. 종이를 자유롭게 돌린다. 그러면서 데이터를 관찰한다. 데이터를 잘 관찰할 수 있는 어떠한 지점이 있는데 거기서 멈춘다. 
2. 멈춘 상태에서 축을 지운다. 
3. 축을 다시 반듯하게 그린다. 즉 축을 재설정한다. 

과정 1-3을 수행하는 것이 회전변환을 수행하는 것과 같다. 
"""

# ╔═╡ e5a5a984-635e-47d0-a859-886a20757dc5
md"""
!!! warning "직교변환" 

	직교변환은 데이터가 정의되는 축을 재설정한다. 이는 데이터를 보는 관점을 재 설정한다는 말과 동일하다. 그런데 왜 이런행동을 할까? 왜 데이터를 다른관점에서 봐야할까? 
"""

# ╔═╡ a9624d52-19b7-49ed-9d5a-80f913ee25ac
md"""
-- 스케일링의 재해석 (데이터는 신축성 있는 가죽모눈종이에 그려져있다고 상상하자.)

1. 가죽을 반듯한 모양으로 늘리거나 줄여보며 데이터를 관찰한다. 이때 모눈종이의 네모모양은 **직사각형** 혹은 정사각형 이어야 한다. (사선으로 늘려서 평행사변형을 만들면 반칙임) 
2. 데이터를 해석하기에 적절하다고 생각하는 어떠한 지점에서 멈춘다. 이때 눈금은 등간격이 아니다. 멈춘다음 모눈종이의 눈금을 모두 지운다. 
3. 모눈종이의 눈금을 정사각형으로 다시 그린다. 

``\star`` 주의: 이 해석은 모든 대각선의 원소가 0 또는 양수일 경우에 성립함. (그런데 SVD를 하면 항상 대각선 원소가 양수임)
"""

# ╔═╡ 56df203b-f6c9-47b1-9790-8f36408a86a0
md"""
!!! warning "스케일링" 

	스케일링은 데이터가 정의되는 축을 눈금을 재설정한다. 스케일링은 역이 존재할수도 하지 않을수도 있다. 일반적으로 역이 존재하지 않는 변환을 취하는 것은 매우 위험하다. 그 이유는 잘못된 변환이 곧 데이터의 손상을 의미하기 때문이다. 따라서 역이 존재하지 않는 스케일링을 데이터에 취하는 행위는 전문가만 할 수 있다. (그런데 이렇게 위험한 변환을 왜 해야할까?) 이러한 전문가를 우리는 통계학자라고 부른다. 통계학자가 "역이 존재하지 않는 스케일링"을 사용하는 경우는 무수히 많지만 가장 많이 사용하는 경우는 아래와 같다. 
	
	- 평균을 구하는 행위
	- ``{\bf y} \approx \hat{\bf y}={\bf X}\hat{\boldsymbol \beta}`` 를 만족시키는 $\hat{\bf y}$을 구하는 행위. (Reconstructing이 불가능함)

	통계의 꽃은 데이터의 요약과 그로 인해 가능하게 되는 적절한 추정이라 볼 수 있는데, 이러한 행위를 가능하게 하는 것이 바로 스케일링이다. 이러한 관점에서 보면 직교변환은 스케일링이 용이하게 수행되도록 밑작업을 해주는 일에 불과하다.
"""

# ╔═╡ 3bdfbaeb-7073-4df2-9c28-bb764de810cc
md"""
!!! warning "디커플링의 재해석"

	두 개의 변수로 정리된 데이터가 표준정규분포를 따른다면, 중심이 밀집된 동그란 구 모양이 된다. 만약 데이터가 타원모양이라면 정규분포가 아니게된다. 자료의 타원모양이 $x$축 혹은 $y$축이 늘어난 모양이라면 두 개의 변수는 서로 분산은 다르지만 상관은 없는 분포라 해석할 수 있다. 그런데 만약에 타원이 방향으로 늘어났다면 두 자료는 상관성을 가진 자료가 된다. 

	하지만 타원을 적당히 돌려서 반듯한 축에 맞춘다면? 그리고 다시 축을 재설정한다면? 자료는 다시 상관이 없어지게 된다. 이후에 스케일링을 수행하면 분산을 맞춰주는 과정이 수행된다. 
"""

# ╔═╡ 03db34bf-5cba-4a75-bd29-05b0c10ea699
md"""
### D. 아이리스 자료와 PCA
"""
...
let 
	...
	function iris_plot(X)
		...
		for i in ["setosa","versicolor","virginica"]
			scatter3d!(X1[y .== i],X2[y .== i],X3[y .== i],alpha=1,label=i,markersize=1)
		end 
		return fig
	end
	U,d,V = svd(X)
	d1,d2,d3 = d
	V1,V2,V3 = eachcol(V)
	D̃ = Diagonal([d1,d2,0])  ## d_3가 엄청 작아서 그냥 0으로 만듦
	Ṽ = [V1 V2 V3*0]  ## V3는 쓸모없게 여김
	iris_plot(X)
	iris_plot(X*V)  ## 직교 변환
...
# ╔═╡ ba819a13-d5ac-4350-88ca-0ffd60085b7f
md"""
$$\bf Z = UD = XV$$
"""

# ╔═╡ 64e837d7-c871-497c-94db-55be6b543635
md"""
`-` 시험, 실험적인 문제를 많이 낼 예정.

> 공식이나 문법을 다 외울 필요는 없을 것 같음.
>
> 수식도 외울 필요는 없음.
>
> 직교변환 같은 게 문제로 많이 나옴, ex) SVD가 통계학과에서 어떻게 해석되는가? 라고 물어볼 때, 데이터가 직교한다? 상관이 없다. 데이터를 직교행렬로 변환했을 때 어떻게 되나? 다른 관점으로 봐진다.
>
> 평소에 생각이 정리가 좀 되어있어야 함. 개념을 깨고 짧은 시간안에 설명할 수 있어야 함.
>
> 결론적으로 그냥 어떤 개념에 대해 아는 대로 쓰라고 할 겁니다.
>
> 종이로 쓰든 웹으로 내든 수식 치는 것 때문에 아무튼 편한대로 하세요.
"""
