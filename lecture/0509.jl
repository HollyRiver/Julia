...
# ╔═╡ 5649ea9b-2e57-4c09-9557-d7ee4e343653
Plots.plotly()

# ╔═╡ 1f43ee21-9e69-4bea-a220-c2d5e1958a8c
md"""
## 3. 아이스크림판매량 ~ 온도
"""
...
# ╔═╡ 4fd83c62-1a2a-4708-b384-557738df628d
...
# ╔═╡ cba4a702-bb38-4794-99d4-6fd07627d474
let
	y = ic1.sales
	x = ic1.temp
	scatter(x, y)
...
# ╔═╡ 2fc567d4-e200-4140-9ead-aad3c9c30749
let
	y = ic1.sales
	x = ic1.temp
	scatter(x, y)
	...
	X = [j x]
	U, d, V = svd(X)
	β̂ = V*inv(Diagonal(d))*U'*y
	plot!(x, X*β̂)
end

# ╔═╡ c8b7cad7-3b75-4dbd-8d23-90310cbd7895
md"""
 $X^{\top}X$는 최소 PSD. $X^{\top}X = VD^2V^{\top}$

 $X = VDV^{\top}$

이를 이용하여 $(X^{\top}X)^{-1} = VD^{-1}V^{\top}$이고, $(X^{\top}X)X^{\top}y = VDV^{\top}y$
"""
...
# ╔═╡ 00e1aef0-6087-4584-b718-b8f5ef3ab502
md"""
*--R-code--*
```r
library(tidyverse)
...
# ╔═╡ 74e800a7-79c6-48d5-a120-6c05a34c295a
md"""
### E. Python을 이용한 분석
"""

# ╔═╡ 0f13d688-3a9c-4f98-98bf-3ce643863ce1
md"""
*--Python-code--*
```Python
import pandas as pd
...
predictor.fit(X,y)
print(f"Coef:{predictor.coef_.round(3)}")
print(f"Intercept:{predictor.intercept_.round(3)}")
```
*--Results--*
```
Coef:[2.516]
...
# ╔═╡ 3b908379-da8f-4422-a580-320b3f5246e0
md"""
## 4. 아이스크림판매량 ~ 온도,Type
"""

# ╔═╡ cc1c502e-d279-4f3e-b532-28c71a086dfc
md"""
### A. Data
"""

# ╔═╡ f49ab0d6-0d22-11ef-1130-83936da33018
ic2 = DataFrame(CSV.File(HTTP.get("https://raw.githubusercontent.com/guebin/SC2024/main/ic2.csv").body))

# ╔═╡ 393b6156-8f78-468d-8494-2e1aa1264452
begin
	j = ones(200)
	## one-hot encoding
	...
	x = ic2.temp
	y = ic2.sales
	X = [j x Choco Vanilla]  ## drop 안한 상태
	[X y]
end
...
# ╔═╡ bea19646-f762-4ea5-af67-e610292610a9
let
	## inv(X'X)*X'y → 풀랭크가 아님
	@show rank(X'X)
...
# ╔═╡ a25bb657-957b-401c-8218-32e21f696dbb
md"""
> 위는 X의 고유값 중 적어도 하나는 0이라는 것을 의미한다. ($\Lambda^{-1}$이 존재하지 않기 위해서)
"""
...
# ╔═╡ 64945a25-d1d0-41ea-a4f2-da2a88b9c33f
let
	U, d, V = svd(X)
	d  ## 마지막 값이 0, 역행렬이 없음. 0인 것은 그냥 무시하고, 나머지만 역을 취하면...?
	d1, d2, d3, d4 = d
...
# ╔═╡ 56fc8508-9dc3-442f-8dbb-37dcc8e8747f
md"""
*--R-code--*
```r
library(tidyverse)
...
Coefficients:
(Intercept)         temp  typevanilla  
     19.629        2.522       20.540  
```
"""

# ╔═╡ 0d2baead-e825-42c2-9256-a81a1d3bb5b4
md"""
*--Julia로 재현--*
"""

# ╔═╡ 224c74a1-4182-4a7b-a1d5-6df4d105727a
let
	X1, X2, X3, X4 = eachcol(X)
	Z = [X1 X2 X4]
	γ̂ = inv(Z'*Z)*Z'*y
...
import sklearn.linear_model
#--#
url = "https://raw.githubusercontent.com/guebin/SC2024/main/ic2.csv"
ic2 = pd.read_csv(url)
#--#
X = pd.get_dummies(ic2[["temp","type"]])
y = ic2["sales"]
predictor = sklearn.linear_model.LinearRegression() 
predictor.fit(X,y)
print(f"Coef:{predictor.coef_.round(3)}")
...
# ╔═╡ 18c9c1ce-cc11-4101-9cd3-979abe66a7c7
let
	X1, X2, X3, X4 = eachcol(X)
	Z = [X2 .- mean(X2) X3 .- mean(X3) X4 .- mean(X4)]  ## 설명변수 텀만 추출
	U, d, V = svd(Z)
	d1, d2, d3 = d
	...
	scatter(x, y)
	scatter!(x, [X2 X3 X4]*γ̂)
end

# ╔═╡ 0ac6b895-6c19-4304-9068-62fbc3ff2e56
let
	X1, X2, X3, X4 = eachcol(X)
	Z = [X2 .- mean(X2) X3 .- mean(X3) X4 .- mean(X4)]  ## 설명변수 텀만 추출
	U, d, V = svd(Z)
...
# ╔═╡ f22f1d5b-47c5-4bf2-8eda-e7050f3c6474
md"""
> 각 계수에 구해진 오차(intercept)를 더하면 회귀분석의 결과와 동일하게 된다.
"""
...
manifest_format = "2.0"
project_hash = "d35f6773b0e0aa54b6d15f6df74966985638f2c8"
