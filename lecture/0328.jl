...
md"""
-- **기: 동원참치회사 주장에 대한 불신 (상식에 대한 의심)** → 가설 제시
- 동원참치를 좋아하는 자취생이 있었음.
...
# ╔═╡ de8350b7-bd81-495c-b4f6-f61d75202865
md"""
$H_0 : \mu = 45 ~ vs. ~ H_1 : \mu < 45$
"""
...
# ╔═╡ ea1387bb-8ecc-4da6-89e8-b748e4b3290e
let 
	n = 30
	X̄ = [rand(Normal(45,1),n) |> mean for i in 1:N]
	x̄ = 44.50 
...
# ╔═╡ 9bc1c18c-14ac-4590-8fb3-4fd1d711db64
md"""
...
# ╔═╡ f92f72a7-ce10-4771-8cc5-23cdcb508822
let
	X̄ = [rand(Exponential(1/θ₀), n) |> mean for i in 1:N]
	(@. 2n*θ₀*X̄) |> histogram
	rand(Chisq(2n), N) |> histogram!

	c1 = quantile(Chisq(2n), 0.025)  ## 상수
	c2 = quantile(Chisq(2n), 0.975)

	L = @. c1/(2n*X̄)  ## L과 U도 확률변수임
	U = @. c2/(2n*X̄)

	(@. L <= θ₀ <= U) |> mean  ## 구간이 θ₀를 포함하는 비율이 얼마인지
	@show (L, U)
end
...
# ╔═╡ f36e9027-935c-4235-b463-01fd6f8340c3
md"""
* 애초에 학생들이 여학생이 많이 올 수도 있음
* 학생들이 친해서 한 교수님에게 몰릴 수도 있음, 그러면 독립성이 깨짐.
"""
