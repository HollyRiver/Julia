...
# ╔═╡ 8e7b7541-9867-4df9-9f07-a12b548a319f
md"""
이전까지는 연산이 중점이었다면 아래는 이해가 중요.
"""
...
md"""
-- 변환을 의미하는 행렬 ${\bf A}$가 데이터를 의미하는 행렬 ${\bf X}$(Design Matrix라고 생각하면 편함) **"앞"**에 곱해지는 경우 ${\bf A}$는 ${\bf X}$의 열별로 적용되는 어떠한 선형변환을 의미한다.
"""
...
# ╔═╡ 41975ed0-cb9d-41a2-928e-9014248f59f5
let
	n = 6
	X1 = [0,0,0,10,10,10]
	X2 = [-10,-10,-10,0,0,0]
...
# ╔═╡ e0b55830-f28e-4169-a9bf-934010198782
md"""
> 합벡터를 n으로 나눈 것 : $\frac1n \bf j$
"""
...
let
	...
	J = j*j'
	X = [X1 X2]
	(I - (J/n))*X ≈ [X1.-mean(X1) X2.-mean(X2)]
end 

# ╔═╡ 038b1e56-dcd9-4012-80ab-54c727be84fd
md"""
> 중심화 행렬 $\bar{C_n}$
"""
...
# ╔═╡ 0e94494c-e279-4ac9-94b8-6e9a8475912f
md"""
> 센터링한 행렬을 전치하여 앞에 곱해주면 됨.
"""

# ╔═╡ b0b156a7-a606-402d-a06b-9a4d61c16c1a
md"""
SSE 계산하는 것과 동일.

$$\bf (I - \frac1nJ)(I - \frac1nJ) = (I - \frac1nJ) : \text{idempotent matrix}$$
...
let 
	Random.seed!(43052)
	n = 100
	t = (1:n)/n  ## 0.01의 간격으로 0~1까지.
	y = sin.(2π*t) + randn(n)*0.2  ## 100×1 vector
	scatter(t,y,alpha=0.2,color="gray")  ## original data
	dl, d, du = ones(n-1), ones(n), ones(n-1)
	M = Tridiagonal(dl, d, du)/3
	plot!(t, (M^k)*y, linewidth = 2, color = "red")  ## k번 이동 평균, 과정이 k번 진행.
end 

# ╔═╡ 4b168f6f-d587-46ec-b597-48a234d14b8e
md"""
> 앞에 $\frac13$을 굵은(?) 대각원소로 가지는 행렬을 곱해주면 된다.
...
# ╔═╡ 64ebcfef-2b52-41f5-8745-12ef26e2a6e6
md"""
> 개별 관측치라고 생각하면 편함
"""
...
# ╔═╡ 3523bd0f-106a-4482-ac3a-739a4b5822b4
let
	scatter(X1, X2, xlim=(-1000,2000), ylim=(-750,1500))
	A = [2.0 0
		 0 	 1.5]  ## 고유치 관점에서 2배, 1.5배
	Z = X*A
	Z1, Z2 = eachcol(Z)
	scatter!(Z1, Z2)
end

# ╔═╡ 609ae0e4-6fde-4909-9734-8f2d960f02bf
md"""
> 대각행렬을 이용하면 좀 wide한 데이터의 변수의 분산을 눌러줄 수 있음.
"""
...
let 
	A = [ cos(θ) sin(θ)
		 -sin(θ) cos(θ)]  ## 극좌표계 변환; 자코비안 행렬이었을걸 아마도...
	Z = X*A
	Z1,Z2 = eachcol(Z)
	scatter(X1,X2, xlim=(-1000,2000),ylim=(-750,1500))	
	scatter!(Z1,Z2)
...
# ╔═╡ 2e2484fb-d821-440e-98dd-53d308e0e013
let
	Random.seed!(43052)
	n = 500
	μ = [0, 0]
	Σ = [2.0 0.6
		 0.6 1.0]
	X = rand(MvNormal(μ, Σ), n)'  ## original : 2×500 matrix
	X1, X2 = eachcol(X)
	p1 = scatter(X1, X2, xlim = (-7, 7), ylim = (-7, 7), alpha = 0.3)  ## 선에 뭉친 것 같고, X1의 분산이 더 큰 것 같음. cov-matrix가 I가 아님.
	Σ̂ = cov(X)
	A = inv(Σ̂)  ## 표준화의 일반화 버전(?)
	Z = X*A
	Z1, Z2 = eachcol(Z)
	p2 = scatter(Z1, Z2, xlim = (-7, 7), ylim = (-7, 7), alpha = 0.3)  ## 아님.
	plot(p1, p2)
end

# ╔═╡ 91eb66a5-d122-486c-b71f-0e17c51728a0
let
	Random.seed!(43052)
	n = 500
	μ = [0, 0]
	...
	X1, X2 = eachcol(X)
	p1 = scatter(X1, X2, xlim = (-7, 7), ylim = (-7, 7), alpha = 0.3)  ## 선에 뭉친 것 같고, X1의 분산이 더 큰 것 같음. cov-matrix가 I가 아님.
	Σ̂ = cov(X)  ## PD. 애초에 X'X/(n-1)의 형태니까 최소 PSD
	λ, Ψ = eigen(Σ̂)
	A = Ψ*Diagonal(1 ./ .√λ)  ## 두배라 빼주는 것 같음. 원래 고유값대로?
	Z = X*A
	Z1, Z2 = eachcol(Z)
	p2 = scatter(Z1, Z2, xlim = (-7, 7), ylim = (-7, 7), alpha = 0.3)
	plot(p1, p2)
end

# ╔═╡ b1ec4b1a-5a47-415c-83f9-f8e565522312
md"""

$$\begin{align}
\hat \Sigma & = \bf X^{\top}\frac{(I - \frac1n J)}{n-1}X \\
\bf Z & = XA = X\Psi\Lambda^{-\frac12} \\
\text{Z의 cov} : & = \bf Z^{\top}\frac{(I - \frac1n J)}{n-1}Z  = \Lambda^{-\frac12}\Psi^{^\top}X^{\top}\hat\Sigma X\Psi\Lambda^{-\frac12} \\

\bf \hat \Sigma
\end{align}$$
"""

# ╔═╡ ed75dd1e-8aa6-4d29-a471-c785ee0d87de
md"""
> X1과 X2가 correlation이 있을 때, 그 관계를 깨고 독립인 것처럼 보이게 만들 수 있음.
"""
...
# ╔═╡ 65306a52-a49f-4685-9ec9-f5fcc5f5fb33
md"""
위에서의 그림이 다 비슷한 것으로 인식되는 이유 : 크기와 모양이 유지가 된다.
"""
...
md"""
-- 어떠한 벡터에 **"직교행렬"**이 변환으로 적용되면, 그 벡터는 크기와 각도가 보존된다.
...
- 각도보전: $\frac{{\boldsymbol x}_1^\top \cdot {\boldsymbol x}_2^{\top}}{\|{\boldsymbol x}_1^\top\|\|{\boldsymbol x}_2^\top\|}=\frac{({\boldsymbol x}_1^\top{\bf A})\cdot ({\boldsymbol x}_2^\top{\bf A})}{\|{\boldsymbol x}_1^\top {\bf A}\|\|{\boldsymbol x}_2^\top{\bf A}\|}$ 이므로 각도도 보존 ($x_1 ㆍ x_2 = ||x_1||||x_2|| cos \theta$)

"""

# ╔═╡ 3fa9ca9e-b566-482e-862e-850f2ec3c560
md"""
> 점들의 상대적인 위치가 동일하다.
"""
...
let
	A = [0.0  1.0 
		 -1.0 0.0]
	@show A'A ≈ I  ## 직교변환인지 체크
	Z = X*A
	Z1, Z2 = eachcol(Z)
	i,j = (2,3)  ## 여기가 어떤 값이 오든지
	X[i, :]'*X[j,:] == Z[i, :]'*Z[j,:]  ## 내적 값이 똑같음.
end 
...
let
	...
	X = [X1 X2 X3] 
	U,d,V = svd(X)
	Z = X*V  ## Design matrix에서 직교변환, X와 동등한 데이터임.
	Z1,Z2,Z3 = eachcol(Z)
	i,j = 1,3
	X[i,:]'X[j,:] ≈ Z[j,:]'Z[i,:]
end 
...
md"""
-- PCA를 해서 얻는 이득: 차원축소의 관점 말고 PCA를 사용해서 얻는 이득이 뭘까? 어차피 동등한 자료라매???

> X를 Z로 바꾸면 Z에 있는 모든 column들이 직교하게 됨???(왜?)
>
> 데이터 축소의 관점 또는 상관을 없애는 관점에서 유용하다.
"""
