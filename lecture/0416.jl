...
# ╔═╡ b524820e-08b3-4c3b-ac0d-6ef9f5ba6fe0
md"""
> 열벡터로 분할한 행렬
"""
...
let 
	B1 = [1,2,3] # (3,1)	## 열벡터
	B2 = [4,5,6] # (3,1)
	B = [B1 B2] # (3,2)  ## 컬럼 벡터 두개를 합쳐서 매트릭스를 바로 선언
	B
end 
...
# ╔═╡ 90d841ac-1879-4596-a97b-7452b5801efd
[1 0
 0 1]
...
- 경우1: 데이터의 저장수단 -- 데이터프레임, 이미지, 회귀분석의 X, ...
- 경우2: 선형변환 (선형연산, 선형사상)
"""
...
# ╔═╡ abd4dc80-ea93-40f7-8bc9-bef82ae4a9a6
begin
	X1 = randn(100)  ## size = (100, 1)인 column vector
	X2 = randn(100) .+1
	X = [X1 X2]
end
...
j=ones(100)  ## sum vector

# ╔═╡ 853b2e64-a587-47c2-8f40-93a41960f65f
[j'X1 j'X2]  ## sum of column vectors, j' is transpose of j
...
# ╔═╡ 5d9bfa68-e881-4221-a1b3-e5387bdfbcb0
md"""
> X1, X2를 데이터라고 할 때, j는 선형 변환이 된다.
"""
...
# ╔═╡ ff87a392-b2e5-47ed-86e1-b277affa0dda
let
	println(j'/length(j)*X)
	println([mean(X1) mean(X2)])
end
...
# ╔═╡ 06a980e0-e89a-47e3-9bb5-f14c3ddb881c
let
	ϵ = randn(100)
	y = 2X1 .+ 3 + ϵ  ## X1, y : given data matrix
	scatter(X1, y, label = "(X1, y)")

	X = [j X1]
	H = X*inv(X'X)X'  ## hat matrix
	scatter!(H*X1, H*y, label = "(HX1, Xy)")
	scatter!(X1, H*y, label = "(X1, Xy)")  ## 똑같음
end

# ╔═╡ dd04d136-83e3-4e5b-b2af-5a7b4c0a5cf0
md"""
> H는 적합된 회귀직선으로 데이터를 변환시키는 사영행렬
"""
...
# ╔═╡ 8070716e-090a-4133-9498-8f741870dd0b
let
	X = [j X1 X2]
	H = X*inv(X'X)*X'
	[H*X X]  ## 당연히 X 붙이면 역행렬 날아가니까...
end

# ╔═╡ 940aca1c-77c8-45db-86e0-eedb158bc8af
md"""
>  $HX = X$에서 $Hj = j, HX1 = X1, HX2 = X2$를 유도할 수 있다.
"""
...
# ╔═╡ 9dde6967-2fd2-4323-9f6d-214d56b4e8ae
RGB(1, 0, 0)
...
# ╔═╡ 7b6d0a61-353b-4d3d-a093-b4caa10bfa12
RGBA(1, 0, 0, 0.5), RGBA(0, 1, 0, 0.5), RGBA(0, 0, 1, 0.5)

# ╔═╡ f564d5b7-d589-4bad-ac5f-c335fdb7f05b
RGB(1,1,0), RGB(1,0,1), RGB(0,1,1)

# ╔═╡ e64ec780-8ffb-48a6-8329-7e139d71f918
...
# ╔═╡ 98b139b6-fb9f-4dad-a74f-d58d46a99347
[RGBA(1, 0, 0, 0.5) RGBA(0, 1, 0, 0.5) RGBA(0, 0, 1, 0.5)
 RGBA(1, 1, 0, 0.5) RGBA(1,0,1,0.5) RGBA(0,1,1,0.5)
 RGB(1,1,1) RGB(0,0,0) RGB(0.5,0.5,0.5)]
...
# ╔═╡ 9fc5ab28-106c-4992-859f-90aa0b1859d8
md"""
* matrix 개체이므로 슬라이싱이 가능
"""
...
md"""
-- 참고 할 기능: channelview(이미지의 색상을 뽑아내는 법)
"""

# ╔═╡ 39353f3c-adac-41b4-a769-07f87f289d67
channelview(France)

# ╔═╡ df65a3d0-54a2-4bbb-85d2-0b64d80601fd
md"""
> 직관적으로 확인 불가능(스택 먹여있음)
"""
...
# ╔═╡ b8b08b23-11c8-4e43-b96c-0eb266b5a4c9
md"""
> 이렇게 보는게 편함
"""
...
# ╔═╡ e333d800-d2ca-4533-a9ce-54d6b88a26aa
md"""
> 매트릭스의 형태로 먹여야 해서 리스트로 묶어줬음
"""
...
md"""
-- 참고 할 기능: colorview(배열을 이미지로 변환)
"""

# ╔═╡ 734bd021-d637-40b0-9ee2-45b27c21aeb2
colorview(RGB, reshape([0.051,0.369,0.686], (3, 1, 1)))

# ╔═╡ b0a9ac58-31d3-4861-8450-5535cf8deccc
...
# ╔═╡ bc3ba676-05af-4189-8a0c-6e1ff25424d9
M = reshape(ones(9)/9, 3, 3)

# ╔═╡ 5a5cfdf0-15b6-4e13-b4bb-92a634287bfd
let
	X0 = zeros(10, 5)
	X1 = ones(10, 5)*3
	X = [X0 X1]
	X[3,3] = 27
	X, imfilter(X, M)  ## edge에 있는 자료들은 알아서 적당히 된다...
	## 컨볼루션은 그냥 매트릭스가 주어지면 점 주변의 값들을 원소별로 곱하고 합, 이동후 반복
end

# ╔═╡ 206a2b60-50a3-4c4b-a7fc-52e06d9994ec
France, imfilter(France, M)  ## 스무딩이 됨

# ╔═╡ e1c2bdbb-f67f-4f03-8b0b-a115dae8beb9
...
G = Kernel.gaussian(1) # 변환을 의미하는 행렬, 가운데가 가중치가 가장 크고 바깥이 작음
...
# ╔═╡ 77c35cb1-d305-4f5b-a397-0d5c3db6023e
France, imfilter(France, G)

# ╔═╡ c2abdc7a-1397-46ae-9fa8-29a3b5264476
...
# ╔═╡ 3d01c3cc-e11e-4a40-aeff-8854cc4c23a6
hani[1500:1600, 1500:1600]
...
# ╔═╡ dad5ac40-7e1b-4472-a857-b00608a2fc05
let
	rt_hani = channelview(hani) .|> (x -> sqrt(x)) |> colorview(RGB)
	[hani' rt_hani']
end
...
# ╔═╡ dc842be6-3e37-420d-a615-3eb7a8c518e6
let
	light_hani = channelview(hani) .|> (x -> sqrt(x)*(x > 0.75) + x*(x<=0.75)) |> colorview(RGB)
	[hani' light_hani']
end
...
# ╔═╡ 9998bdd7-26f7-49ba-adc7-401090381e00
let
	smooth_hani = imfilter(hani, Kernel.gaussian(20))
	[hani' smooth_hani']
end
...
# ╔═╡ 5f2bd2b8-15d9-4f98-9451-0296f8561ce8
let
	smooth_hani = imfilter(hani, Kernel.gaussian(30))
	sharp_hani = hani*2 - smooth_hani
	[hani' smooth_hani' sharp_hani']
end
...
manifest_format = "2.0"
project_hash = "3f2bd97ec957231832994aee77415bbe28dba70a"
