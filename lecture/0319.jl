...
#> risky_file_source = "https://guebin.github.io/SC2024/03wk-1.jl"
...
# ╔═╡ 90114466-a631-4ef4-97ea-910f4d326570
md"""θ = $@bind θ Slider(0.01:0.01:10, show_value = true, default = 1)"""

# ╔═╡ 53026c6d-5319-4bb0-b5b1-63b58cfd5405
let
	N = 10000
	p1 = rand(Exponential(θ), N) |> histogram; xlims!(0, 100)  ## histogram
...
# ╔═╡ 06cb0e2b-098f-47cf-8663-9c6e013e6308
md"""
베르누이 시행 -> 첫 번째 1이 나올 때까지 몇 번의 시도를 해야 하는지를 카운트
"""
...
# ╔═╡ 3f5fa18a-f62b-498b-828a-306d25757c45
function mygeo(p)
	u = rand()
...
# ╔═╡ a7e620e2-4b8f-4ffd-b24e-92a5cf606076
let
	N = 10000
	p = 1/2
	p1 = [mygeo(p) for i in 1:N] |> histogram; xlims!(0, 15)  ## 만들어낸 분포
	p2 = rand(Geometric(p), N) |> histogram; xlims!(0, 15)  ## 이론적인 분포(0부터 시작함)
...
# ╔═╡ 874e9967-c151-4288-ba0f-d2ca67bff335
let
	λ = 4
	θ = 1/4
	n = 1000  ## 단위시간 1을 1000등분
	Δt = 1/n   ## 쪼개어진 시간
	p = λ/n   ## np = λ
	...
	p2 = rand(Exponential(1/λ), N) |> histogram; xlims!(0,5)
	plot(p1,p2)
end
...
# ╔═╡ 4ea9eed3-d648-4a88-bff1-c85b3b382726
let
	f(x) = exp(-x)
	g(x) = 1/5*exp(-1/5*x)
...
# ╔═╡ 304c366b-fab3-4303-b770-25cd23099867
let
	p1 = plot(x -> 1-exp(-x), 0, 20)
	p2 = plot(x -> 1-exp(-1/5*x), 0, 20)
...
`-` cdf의 y축에서 랜덤변수를 발생시킨다음 $\rightarrow \downarrow$ 와 같이 이동하여 $x$축에 내린다고 생각해보자. : y의 함수를 취하면...
- 왼쪽: 대부분 5이하에 떨어짐 
...
# ╔═╡ 17cbd199-dbfa-4b19-a26c-e39fbe54d45d
let
	## -x, exp, -x, +1
	F(x) = 1-exp(-x)
	Finv(x) = -log(1-x)
	...
	U = rand(5)
	@show U
	p1 = plot(F, 0, 20); scatter!([0,0,0,0,0], U); scatter!(Finv.(U), [0,0,0,0,0])
	p2 = plot(G, 0, 20); scatter!([0,0,0,0,0], U); scatter!(Ginv.(U), [0,0,0,0,0])

	plot(p1, p2)
end

# ╔═╡ 2ea4e1d5-dc13-410f-9477-bb3d265a4915
md"""
함수의 브로드캐스팅
"""

# ╔═╡ ee8ef2a1-6fd2-44a8-aff5-292dec078a6e
let
	f = x -> x+1
	f.([1,2,3])  ## .을 붙인다.
end
...
# ╔═╡ 24dd44bf-780c-4494-8151-f81b40a4d14c
let
	N = 10000
	Finv(x) = -log(1-x)
	rand(N) .|> Finv |> histogram
	rand(Exponential(1), N) |> histogram!  ## 두 개의 히스토그램이 겹쳐있다. 즉, 누적분포함수의 역함수는 확률밀도함수와 동일하다.
end

# ╔═╡ 84ff7e72-5d59-413a-9a8c-9a796abb0918
let
	N = 10000
	Ginv(x) = -5*log(1-x)
	Ginv.(rand(N)) |> histogram
	rand(Exponential(5), N) |> histogram!
end
...
# ╔═╡ e1555f50-96d7-4b50-913e-f75b68cb4cf7
md"t = $@bind t Slider(0.01:0.01:5, show_value = true, default = 1)"

# ╔═╡ 0107bbf3-709f-4d10-ab1f-7d48754c2f78
md"s = $@bind s Slider(0.01:0.01:5, show_value = true, default = 2)"

# ╔═╡ 997aeef9-0bb6-4070-8d97-2fd4b09869dd
# P(X>t) = P(X>t+s|X>t)

# ╔═╡ d63fff11-459d-4a6d-9fdf-0189913cc06b
let
	N = 1000
	X = rand(Exponential(5), 1000)
...
# ╔═╡ edeb6a13-d827-4b02-8f3a-1d12558a3deb
let
	N = 10000
...
# ╔═╡ 99bf7eea-ebcf-4980-b1d5-66234bcc2cdb
let
	## 반지름의 제곱은 지수분포이다.
	Random.seed!(43052)
	X = randn(5000)
	Y = randn(5000)
	xi, yi = X[i], Y[i]
	p1 = scatter(X, Y, alpha = 0.1); plot!([0,xi], [0,yi], linewidth = 5)
	p2 = (@. X^2 + Y^2) |> histogram; xlims!(0, 25); rand(Exponential(2),5000) |> x-> histogram!(x,alpha=0.5,label="평균2인 지수분포")
	plot(p1, p2)
end
...
# ╔═╡ f5a5e6ed-34f7-4010-bce2-20d843f71262
md"""
1. unif에서 누적분포함수의 역함수를 이용해 지수분포를 뽑고, 제곱근을 취함(반지름으로 만듦)
2. cos()을 취하여 극좌표계 변환
"""
...
# ╔═╡ c4827fac-83b0-4a28-bb83-0b8dc87302e2
let
	N = 10000
	U1 = rand(N)
	U1 = rand(N)

	X = @. √(-2log(1-U1)) * cos(2*π*U2)
	Y = @. √(-2log(1-U2)) * sin(2*π)
