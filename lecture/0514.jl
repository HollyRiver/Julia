...
# ╔═╡ 3cdfc89d-ff70-402b-bb88-c99c77e783a4
md"""
> 대충 행마다 미분때림. (대단한 해석)
"""
...
# ╔═╡ 3c07de98-cee4-4bcd-9d0a-4f2f41431159
md"""
> m × 1 벡터를 미분하면 n×m 행렬이 됨.
"""
...
# ╔═╡ 9d52017d-f12c-4e85-bf39-625c4328d3e8
md"""
1)  $\frac{\partial}{\partial \bf x}\bf x^{\top} \bf y = \bf y$
...
# ╔═╡ 52d83fd9-7f28-4e36-8fbf-0fa2caa8a9c8
md"""
> 풀이 2의 경우 편리해보이긴 하나, 차수가 1이 아닌 상황에서 문제가 생길 수 있음.
"""
...
# ╔═╡ 04b9cfa7-37f8-496a-83ed-80dab570addc
md"""
$$\boldsymbol{\hat \beta} = (\bf X^{\top}X + \lambda \bf I)^{-1} \bf X^{\top}y$$

> 위에서 $(\bf X^{\top}X + \lambda \bf I)$의 역행렬은 항상 존재한다. 왜냐하면...
>
> $$\begin{align}\bf X^{\top}X & \Rightarrow PD ~ or ~ PSD , ~ let ~ \bf X^{\top}X ~ is ~ PSD\\
> \bf \Psi \Lambda \Psi^{\top} & = \bf \Psi \begin{bmatrix}
> 	\lambda_1 ~ \cdots ~ 0 ~ 0 \\
>  	\ddots \\
>   0 ~ \cdots ~ \lambda_n ~ 0 \\
> 	0 ~ \cdots ~ 0 ~~~ 0
> \end{bmatrix} \bf \Psi^{\top} \\
> \lambda \bf I & = \bf \Psi I \Psi^{\top} \\
> \bf \Psi(\Lambda - \lambda \bf I)\Psi^{\top} & \Rightarrow diag\{\Lambda - \lambda I \} > 0 ~ \therefore ~ (\Lambda - \lambda \bf I) ~ is ~ PD
> \end{align}$$
"""
...
# ╔═╡ 0ff41c21-4923-4369-afc5-c60885bdb596
md"""
> 벡터 미분에서도 Chain Rule이 성립한다.
>
> $$\frac{\partial}{\partial \boldsymbol \beta} loss =\bigg(\frac{\partial}{\partial \boldsymbol \beta}{\boldsymbol u}^\top\bigg)\bigg(\frac{\partial}{\partial \boldsymbol u}{\boldsymbol v}^\top\bigg)\bigg(\frac{\partial}{\partial \boldsymbol v}loss\bigg) = \frac{\partial}{\partial \bf{\beta}} \frac{\partial \bf u^{\top}}{\partial \bf u} \frac{\partial \bf v^{\top}}{\partial \bf v} loss$$
"""
