...
# ╔═╡ 83c32d06-8eca-4b78-9dcc-c649b078620a
md"""
* 수학 : 수학문제를 많이 풀면서 빠르게 잘 계산하는 능력?

> 수학책 : 논리체계를 소개하는 책, 논리전개와 일반화, 실전 적용


`-` 선형대수학? : 행렬을 계산하는 방법도 다루면서, 벡터공간 등등...
"""
...
# ╔═╡ 7f6de85d-3f5a-448d-8035-0b036183d751
md"""
* 모순을 유도해서 거짓을 증명
"""
...
# ╔═╡ 2f3f78fe-4abd-444c-a023-4d620cb6a305
md"""
> 이미 알려진 개념을 확장해서 새로 정의
"""
...
# ╔═╡ 7a0834b7-2cfb-490e-9193-3e5037a32768
let
	X = [0 1 0
		 0 0 1
		 1 0 0]
...
md"""
-- 예시2 각 열에 음수를 같이 취해도 동일
"""

# ╔═╡ 46170ae4-9f47-4cb6-9c96-fc740723d004
let
	X = [0 1 0
	 	 0 0 1
	 	 1 0 0]
	...
	Ũ = [U1 U2 -U3]  ## 같은 열에 마이너스
	Ṽ = [V1 V2 -V3]

	@show Ũ*Ũ' ≈ Ũ'*Ũ ≈ I
	@show Ũ * Diagonal(d) * Ṽ ≈ X  ## 생각해보면 당연한거...
end
...
# ╔═╡ 3c44ad37-5e7a-41a6-afaa-efc4fa60d677
let
	X = [2 0
		 0 -1]
	λ, Ψ = eigen(X)
...
# ╔═╡ 69d6d71e-22c8-4e06-bdd6-0618280b42c0
md"""
> 고유값에 음수가 있으므로 SVD와 대응될 수 없음...
"""

# ╔═╡ a8f3b2e5-7a87-4ec5-bb90-b8eded4779fe
let
	X = [2 0
		 0 -1]
	λ, Ψ = eigen(X)
	...
	d = λ
	@show U'*U ≈ U*U' ≈ I
	@show U*Diagonal(λ)*V' ≈ X
end

# ╔═╡ ba86f635-2b03-4c8e-a570-ebec248b3341
md"""
> U, D, V는 X = UDV'를 만족하지만, SVD의 분해결과로 인정하지 않음.
"""

# ╔═╡ 03a0bf31-6802-4dfa-8f5f-49d800578ef4
md"""
`-` 그럼 해당 벡터의 한쪽만 부호를 바꾸면 되지 않을까?
"""

# ╔═╡ 6c98f658-3a96-4d91-b646-c6ede497002e
let
	X = [2 0
		 0 -1]
	λ, Ψ = eigen(X)
	...
	@show U*Diagonal(d)*V' ≈ X
end

# ╔═╡ 1c138ea2-dea0-4c0f-af40-89ed14d24ad5
md"""
> 만족하면서 SVD의 분해결과로 인정됨!
"""
...
# ╔═╡ 0d8debf9-ff95-4e2c-96b7-63af16439361
md"""
> 고유값은 1이고, 고유벡터는 같은 차원에서 정의된 0이 아닌 모든 벡터
"""
...
# ╔═╡ 3a31add8-6532-41ed-85b7-952a6946a7b6
md"""
> 고유값은 0이고, 고유벡터는 같은 차원에서 정의된 0이 아닌 모든 벡터
"""
...
# ╔═╡ 8ba9ddd7-262c-49a7-8fbb-b2fc2c0581a4
md"""
> 고유값은 대각원소, 고유벡터는 같은 차원에서 정의된 0이 아닌 모든 벡터
"""
...
# ╔═╡ 3d2e886d-5053-4d56-9787-f9ca7694a2ac
md"""
> 표준화하더라도 최소 두개...
"""
...
# ╔═╡ 0a122fe9-f7bc-4712-94f1-ef5903d3ad9e
md"""
유사표준형...
...
# ╔═╡ dd54ecf4-0253-4a0d-b689-394a84665de0
md"""
> 전부 더했을 때, 0이 되는 $\boldsymbol{\psi}$가 0-벡터밖에 없음.
"""
...
# ╔═╡ 37560e31-1e0f-4423-a89c-3dbd68058cde
md"""
> 고유벡터가 없으면 선형독립인데, 고유값이 있으려면 선형종속이여야 함. 따라서 말도 안됨
"""
...
# ╔═╡ 68e9a437-5e34-45eb-a128-955381dc7220
md"""
> 중근인 상황, 둘다 0으로 안나온 건 컴퓨팅 때문인가?
"""
...
# ╔═╡ e49f240b-b590-4d2d-ae19-706d838b3c73
md"""
> A에 Ψ라는 변환을 하면 대각행렬이 되었다. A는 대각화가 가능하다.
"""
...
# ╔═╡ afd9c3d9-2275-4e02-bb89-dbeedfdb67ec
md"""
> 역행렬이 존재하므로 당연.
"""
...
md"""
!!! info "Fact2"
	고유값이 중근을 가진다면 (근이 2개 겹친다면), 대응하는 고유벡터행렬의 rank는 $1$일수도 있고 $2$일수도 있다. 만약에 고유값이 $m$개의 중첩된 근을 가진다면 고유벡터행렬의 rank는 최소 $n-m+1$ 최대 $n$이다. 
"""

# ╔═╡ 2bf09619-a054-46bf-96a8-0c8cca0635df
md"""
> 항등행렬의 경우 full-rank가 되도록 잡을 수 있음.
"""
...
# ╔═╡ 3b701de6-809b-495e-b9f8-a9bc98185058
md"""
+ 실대칭인 양정치행렬이면 가능
"""
...
# ╔═╡ 41289c7f-6808-412a-8c27-55f94147ab0a
md"""
> 영행렬의 고유값은 0이고, 대응하는 고유벡터는 2차원 유클리드공간에서의 모든 순서쌍이 가능하다. 따라서, A의 고유벡터들이 생성하는 공간은 2차원이 된다.
"""
