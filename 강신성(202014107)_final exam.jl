...
> * 디자인 매트릭스 $\bf X_{n\times p}$의 SVD를 이용하면 적당한 직교변환을 수행하여 원래의 행렬로 복원할 수 있는 행렬을 만들 수 있다. 이 행렬의 각 열을 주성분이라고 하고, 이 행렬을 이용하여 분석하는 기법을 주성분분석이라고 한다.
>
> * 주성분분석은 디자인 매트릭스 자체를 활용하는 것보다 차원이 축소된다는 점, 변수 간 직교하는 형태로 만들어 다중공선성을 해소시켜준다는 점에서 유리하다. 특히, 다중공선성이 있는 상황에서 주성분회귀로 추정된 회귀계수의 MSE가 최소제곱추정량의 MSE보다 작다는 점에서 유용하다.

---

##### **B. 회귀분석에서의 활용**

> * 디자인 매트릭스의 SVD는 회귀분석의 계수를 구할 때에 활용할 수도 있다. 이 경우, 특이값의 역수를 이용하여 역행렬을 계산한다. 설명변수 간 선형종속 관계가 있을 경우 0으로 산출되는 특이값을 역수를 취하지 않고 그대로 사용하는 일반화역행렬의 꼴을 이용하여 회귀계수를 추정할 수 있다는 점에서 범용성이 크다.
