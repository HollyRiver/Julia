...
*  $\mathbb{V}(\boldsymbol{\hat \beta}) = \bf (X^{\top}X)^{-1}\sigma^2$이므로, 이는 $\bf (VD^2V^{\top})^{-1}\sigma^2 = V(\sigma^2D^{-2})V^{\top}$로 나타낼 수 있다. 즉, 고유벡터행렬은 $\bf V$인데, $\bf XV = Z$이므로 설명변수 행렬$\bf X$의 모든 열들이 직교하게 변환할 수 있다. 이는 변수 간 상관관계가 없도록 만드는 통계적 처리기법으로서 유용하다.
...
*  $\bf A$가 변환을 의미하는 행렬이라고 하자. 그러면 이를 다음과 같은 특이값 분해 표현으로 나타낼 수 있다.
$\bf A = UDV^{\top}$

* 이는 어떠한 데이터에 대하여 정의되는 축을 재설정하고, 해당 축에 대하여 스케일링을 수행한 뒤 다시 직교변환을 수행하는 것으로 분해하여 해석할 수 있다.
