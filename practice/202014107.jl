...
# ╔═╡ d56099c8-eae5-4469-a75f-ad3eb07e49ab
Plots.plotly()

# ╔═╡ cca86a17-9044-4f82-a6a5-32319158f191
PlutoUI.TableOfContent()
...
# ╔═╡ 49b98204-454b-491f-aac1-9e11fb3d4893
...
# ╔═╡ 03786fba-1ce8-4e81-ba8c-0de1669b5672
let
	n = 1000
	N = 1000
	p = 0.4
	...
	println("μ에 대한 95% 점근적 신뢰구간(시뮬) : $((quantile(X̄, 0.025), quantile(X̄, 0.975)))")
	println("μ에 대한 95% 점근적 신뢰구간(델타 메소드) : $((quantile(Normal(p, σ), 0.025), quantile(Normal(p, σ), 0.975)))")
end

# ╔═╡ b50ebb92-e87e-463f-8f9a-b8eed615c289
let
	n = 1000
	N = 1000
	p = 0.4
	...
	println("σ에 대한 95% 점근적 신뢰구간(시뮬) : $((quantile(Y, 0.025), quantile(Y, 0.975)))")
	println("σ에 대한 95% 점근적 신뢰구간(델타 메소드) : $((quantile(Normal(μ, σ), 0.025), quantile(Normal(μ, σ), 0.975)))")
end
...
# ╔═╡ fddda6af-df24-4c4a-ba51-95ad4bfde975
let
	g(x) = asin(sqrt(x))
	n = 1000
	N = 1000
	...
	println("g(p̂)의 95% 점근적 신뢰구간(시뮬) : $((quantile(Y, 0.025), quantile(Y, 0.975)))")
	println("g(p̂)의 95% 점근적 신뢰구간(델타 메소드) : $((quantile(Normal(μ, σ), 0.025), quantile(Normal(μ, σ), 0.975)))")
end
...
# ╔═╡ f893babb-213b-4930-b46b-339d041be11f
md"p = $(@bind p Slider(0.1:0.1:0.9, show_value = true, default = 0.2))"

# ╔═╡ 3429c20e-50bd-4684-9802-4f5305f7d51d
let
	g(x) = asin(sqrt(x))
	n = 1000
	N = 1000
	...
	println("g(p̂)의 분산(시뮬) : $(var(Y))")
	println("g(p̂)의 분산(델타 메소드) : $(σ^2)")
end

# ╔═╡ 46d1a526-2513-4e9c-8d2c-686e1222643e
md"""
> p에 따른 분산 변화가 없다.
"""
...
# ╔═╡ ea7064b4-fb43-4a8f-a4d9-db05c371ff91
md"""
...
# ╔═╡ 6ee78263-aad6-4ebd-816b-663bb79fff3e
let
	N = 10000
	n = 30
	...
	println("p-value(이론) = $(1-cdf(Normal(0, 1/sqrt(n)), x̄))")
	println("p-value(시뮬) = $(mean(X̄ .>= x̄))")
end

# ╔═╡ 1ec7c95d-2874-4b8a-9072-19509388b48f
md"""
...
# ╔═╡ 70bc390d-dd02-43a6-9dc3-1a2d546e5d09
md"n = $(@bind n Slider(850:870, show_value = true))"

# ╔═╡ 0354981b-29b7-4dd6-a682-34e27d227cb7
println("95% CI : $((quantile(Normal(0, 15/sqrt(n)), 0.025), quantile(Normal(0, 15/sqrt(n)), 0.975)))")

# ╔═╡ 5d5632f2-5961-4a0a-b4ac-cb00ff4c010c
md"""
> 865
"""

# ╔═╡ 638c7bf1-ae49-49bc-81fd-377d72ba2aaa
md"""
**(3)-(5)**
...
# ╔═╡ 97580c33-8a1f-4626-a089-fe116a26db4e
let
	n = 3
	N = 10000
...
# ╔═╡ fe865b9d-feb0-452e-a515-071aa93cf15c
md"""
(4) 주어진 검정통계량의 $p$-value의 이론값과 시뮬레이션값을 각각 구하고 비교하라. 
"""

# ╔═╡ 8bf1907f-8661-4a70-b805-2b1c83ab2f36
let
	n = 3
	N = 10000
	X̄ = [rand(Normal(0, 1), 3) |> mean for i in 1:N]
	...
	println("p-value(이론) : $(1-cdf(Exponential(2/3), r_stat))")
	println("p-value(시뮬) : $(mean(r_sq .>= r_stat))")
end

# ╔═╡ 994b0859-8c77-442a-b28f-f87d7fa05c84
md"""
...
# ╔═╡ bf147b9e-a4e7-4502-b19b-042e7807e629
let
	n = 30
	N = 10000
	X̄ = [rand(Normal(0, 1), 30) |> mean for i in 1:N]
	Ȳ = [rand(Normal(0, 1), 30) |> mean for i in 1:N]
	...
	println("n = 30의 p-value(이론) : $(1-cdf(Exponential(2/30), r_stat))")
	println("n = 3의 p-value(이론) : $(1-cdf(Exponential(2/3), r_stat))")
end
